...
suite('models/message', (s) => {

	s.after(() => neo4j.end());

	s.test('getById returns all children and parents', (t) => bootstrap({ depth: 6 }).then((conditions) => {
		var message = conditions.tails[0].parent.parent.parent;
		return Promise.props({
			secondChild: bootstrap.createMessage(null, message),
		})
			.then(() => Message.getById(message.id, 2))
			.then((result) => schemas.validate(result, schemas.model.message))
			.then((result) => {
				bootstrap.inspect(result);
